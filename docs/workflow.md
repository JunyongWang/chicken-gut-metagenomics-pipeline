...
The diagram shows the verified production relationships through Stage 13.

```mermaid
flowchart TB
    accTitle: Chicken gut metagenomics workflow through Stage 13
    accDescr: Read-level profiling and genome-resolved analysis feed community and MAG gene catalogs, Stage 12 functional annotation, and Stage 13 KO abundance outputs.
    ...
    subgraph functional ["Functional characterization"]
        ...
        community_catalog --> eggnog_nr["12A eggNOG NR"]
        eggnog_nr --> dbcan_nr["12B dbCAN NR"] --> nr_cazyme["NR CAZyme-family abundance"]
        community_gene_abundance --> nr_cazyme
        eggnog_nr --> nr_ko["13A NR KO abundance"]
        community_gene_abundance --> nr_ko

        representative_mags["374 representative MAGs"] --> mag_catalog["11B MAG-specific catalog"]
        mag_catalog --> eggnog_mag["12A eggNOG MAG"]
        eggnog_mag --> dbcan_mag["12B dbCAN MAG"] --> mag_cazyme_counts["MAG × CAZyme family counts"]
        eggnog_mag --> mag_ko_counts["13B MAG × KO counts"]

        mag_abundance --> weighted_cazyme["MAG-weighted CAZyme TPM"]
        mag_cazyme_counts --> weighted_cazyme
        mag_abundance --> weighted_ko["13C MAG-weighted KO TPM"]
        mag_ko_counts --> weighted_ko
    end
...
Verified stage pages: [01](stages/01-fastp.md), [02](stages/02-dehost.md), [03](stages/03-taxonomy.md), [04](stages/04-assembly.md), [05](stages/05-mapping-depth.md), [06](stages/06-binning.md), [07](stages/07-checkm2.md), [08](stages/08-drep.md), [09](stages/09-gtdbtk.md), [10](stages/10-coverm.md), [11](stages/11-gene-catalog.md), [12](stages/12-functional-annotation.md), and [13](stages/13-functional-abundance.md).
