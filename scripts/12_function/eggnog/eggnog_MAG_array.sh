#!/bin/bash
#SBATCH --job-name=eggnog_MAG
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=24G
#SBATCH --time=24:00:00
#SBATCH --output=logs/eggnog_MAG_%A_%a.out
#SBATCH --error=logs/eggnog_MAG_%A_%a.err
...
EGGNOG_MAG_CHUNKS_DIR="${EGGNOG_MAG_CHUNKS_DIR:-$STAGE_DIR/eggnog/11B/chunks}"
EGGNOG_MAG_CHUNKS_FILE="${EGGNOG_MAG_CHUNKS_FILE:-$STAGE_DIR/eggnog/11B/chunks.txt}"
...
CHUNK=$(sed -n "${SLURM_ARRAY_TASK_ID}p" "$EGGNOG_MAG_CHUNKS_FILE")
...
IN="$EGGNOG_MAG_CHUNKS_DIR/$CHUNK"
OUTDIR="$STAGE_DIR/eggnog/11B/results/$BASE"
...
echo "============================================================"
echo "eggNOG MAG formal annotation"
echo "Chunk    : $CHUNK"
echo "Node     : $(hostname)"
...
printf "Chunk\tInput_proteins\tAnnotated\tSeed_orthologs\tAnnotation_rate_percent\n" \
    > "$QC"
